...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    float time_spent = (stop_time-start_time)/125e6; 
	throughput = ((float)(bytes_transferred)/1024/1024)/time_spent;
    printf("You transferred %i bytes of data in %f seconds\n",bytes_transferred,time_spent);
    printf("Measured Transfer throughput = %f Mbytes/sec (Mbyte = 2^20 bytes)\n",throughput);
}
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 David Kaplan - Custom Peripheral Demonstration\n\r");
